...
export const AiCareerChatAgent = createAgent({
  ...
  model: gemini({
    ...
    apiKey: process.env.AI_CAREER_CHAT_API_KEY!,
  }),
});
...
export const AiResumeAgent = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    //uploading the file to cloud using imagekit.
...
export const AIRoadmapGeneratorAgent = createAgent({
  ...
  description:
    "Generates a flow tree-structured learning roadmap for position or skills.",
...
export const AIRoadmapAgent = inngest.createFunction(
  ...
  async ({event, step}) => {
    const {roadmapId, userInput, userEmail, aiAgentType} = await event.data;
